...
# 0. Ensure target data directories exist
...
if [ ! -f "backend/data/hrms.sqlite" ] || [ ! -s "backend/data/hrms.sqlite" ]; then
  if [ -f "backend/data/database.sqlite" ]; then
    ...
    if [ -n "$LATEST_BACKUP_DB" ]; then
      ...
      cp "$LATEST_BACKUP_DB" backend/data/hrms.sqlite
    fi
  fi
else
...
chmod -R 777 backend/data backend/uploads 2>/dev/null || true
...
if [ ! -f "backend/data/hrms.sqlite" ] || [ ! -s "backend/data/hrms.sqlite" ]; then
  ...
  if [ -n "$RESTORE_DB" ]; then
    ...
    cp "$RESTORE_DB" backend/data/hrms.sqlite
  fi
fi
